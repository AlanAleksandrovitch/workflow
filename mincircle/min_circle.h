...
struct Point2D {
    ...
    bool operator==(const Point2D& other) const {
        return std::fabs(x - other.x) < 1e-8 && std::fabs(y - other.y) < 1e-8;
    }
...
struct Circle {
    ...
    bool contains(const Point2D& point) const {
        ...
        return isValid() && std::sqrt(dx * dx + dy * dy) <= radius + 1e-8;
    }
    ...
    bool containsEntireSegment(const LineSegment& segment) const {
        Point2D mid{(segment.start.x + segment.end.x) / 2, (segment.start.y + segment.end.y) / 2};
        return contains(segment.start) && contains(mid) && contains(segment.end);
    }
};
